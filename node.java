...
public class node {
	...
	public node(String input, int goal) {//initial starting board state node
		record = input;
		this.goal = goal;
		goalState = GoalCheck(input, goal);
	...
	public node(String input, node par, int c, String prevMov, int goal, int indexOfSpace, int d, int hv) {//board state node in the tree of possible moves. 
		record = input;
		this.goal = goal;
		goalState = GoalCheck(input, goal);
		boardState = createArr(input);
	...
	public boolean GoalCheck(String value, int goal) { //Checks to see if the board state has reached the goal state
		boolean isGoal = false;
		if(Integer.valueOf(value) == goal) {
			isGoal = true;
		}
		return isGoal;
	}
	
	public void createChildArr(ArrayList<node> childA, String possMoves, int [] boardState, int goal, int ios, int depth, node p, int heuristicValue) {//based on a node of a board state, it will create children node of board states and add it to the arraylist of board states
		...
		if(possMoves.contains("L")) {
			...
			childA.add(new node(output, p, piece, "L", goal, ios+1, depth, heuristicValue));
		...
		if(possMoves.contains("R")) {
			...
			childA.add(new node(output, p, piece, "R", goal, ios-1, depth, heuristicValue));
		...
		if(possMoves.contains("U")) {
			...
			childA.add(new node(output, p, piece, "U", goal, ios+3, depth, heuristicValue));
		...
		if(possMoves.contains("D")) {
			...
			childA.add(new node(output, p, piece, "D", goal, ios-3, depth, heuristicValue));
